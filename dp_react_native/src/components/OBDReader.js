...
export default class OBDReader extends Component {
  ...
  componentDidMount() {
    this.btStatusListener = DeviceEventEmitter.addListener(
      'obd2BluetoothStatus',
      this.btStatus,
    );
    this.obdStatusListener = DeviceEventEmitter.addListener(
      'obd2Status',
      this.obdStatus,
    );
    ...
    this.setDeviceAddressListener = AppEventEmitter.addListener(
      'OBDReader.setDeviceAddress',
      this.setDeviceAddress.bind(this),
    );
  ...
  startLiveData() {
    SharedPreference.getBoolean(Constant.KEY_ENABLE_MOCKUP).then(
      isMockUpMode => {
        ...
        this.listenerOrientation = DeviceEventEmitter.addListener(
          'Orientation',
  ...
  render() {
    let startLiveColor = this.state.isStartLiveData
      ? Color.DISABLED_COLOR
      : Color.BLACK;
    let stopLiveColor = this.state.isStartLiveData
      ? Color.BLACK
      : Color.DISABLED_COLOR;

    let originData = this.state.obd2Data;
    let cmdKeys = Object.keys(this.state.obd2Data);
    let cmdData = cmdKeys.map(function (key) {
              ...
              <Menu onSelect={this.runMenu.bind(this)}>
                ...
                <MenuOptions>
                  <MenuOption disabled={this.state.isStartLiveData} value={1}>
                    <Text
                      style={[styles.menuOptionText, {color: startLiveColor}]}>
                      Start Live Data
                    </Text>
                  </MenuOption>
                  <MenuOption disabled={!this.state.isStartLiveData} value={2}>
                    <Text
                      style={[styles.menuOptionText, {color: stopLiveColor}]}>
                      Stop Live Data
                    </Text>
                  </MenuOption>
                  <MenuOption value={3}>
              ...
              <ScrollView>
                {cmdData.map((item, index) => (
                  <View
                    ...
                    key={index}>
                    <Text style={{flex: 0.6, textAlign: 'right'}}>
                      {item.cmdName}
                    </Text>
                    <Text style={{flex: 0.4}}>: {item.cmdResult}</Text>
                  </View>
                ))}
              </ScrollView>
            </View>
